...
def CMAL(mmil_logits, audio_logits, visual_logits, seq_len, audio_rep, visual_rep):
    device = visual_rep.device
    # 2. 使用正确的设备来创建所有新的张量
    audio_abn = torch.zeros(0, device=device)
    visual_abn = torch.zeros(0, device=device)
    audio_bgd = torch.zeros(0, device=device)
    visual_bgd = torch.zeros(0, device=device)
    ...
    if audio_nor.size(0) == 0 or audio_abn.size(0) == 0:
        ...
        cmaloss_v = cmals(visual_abn, visual_abn, visual_nor)
        cmaloss_a = cmals(audio_abn, audio_abn, audio_nor)
        ...
        return loss_a2v_a2b, loss_a2v_a2n, loss_v2a_a2b, loss_v2a_a2n, cmaloss_v, cmaloss_a
