...
def anomap(predict_dict, dataset, save_root):
    ...
    # --- 【新增核心修改 1】 计算全局统计阈值 ---
    # 为了进行分级预警，我们需要先收集所有视频的预测分数来计算分布统计量
    print("正在计算全局阈值 (基于分位数)...")
    all_scores_list = []
    for item in predict_dict:
        # 获取预测分数，确保转为 numpy 数组并展平
        scores = item['pre_dict']
        if isinstance(scores, torch.Tensor):
            scores = scores.detach().cpu().numpy()
        all_scores_list.append(scores.flatten())

    if len(all_scores_list) > 0:
        all_scores_concat = np.concatenate(all_scores_list)
        # 计算统计量：中位数、75%分位数、95%分位数
        th_median = np.median(all_scores_concat)
        th_75 = np.percentile(all_scores_concat, 75)
        th_95 = np.percentile(all_scores_concat, 95)
        print(f"--- 阈值计算完成 ---")
        print(f"   [低预警] 中位数 (Median): {th_median:.4f}")
        print(f"   [中预警] 75% 分位数: {th_75:.4f}")
        print(f"   [高预警] 95% 分位数: {th_95:.4f}")
    else:
        # 如果没有数据，使用默认兜底值
        th_median, th_75, th_95 = 0.5, 0.7, 0.9
        print("警告：未收集到预测数据，使用默认阈值。")
    # -------------------------------------------
    ...
    for i, item in enumerate(predict_dict):
        ...
        # 使用不同的颜色和线型表示不同的预警等级
        # 1. 低风险/基准线 (中位数) - 金色
        plt.axhline(y=th_median, color='#FFD700', linestyle='--', linewidth=1, alpha=0.8,
                    label=f'Median ({th_median:.2f})')
        # 2. 中风险线 (75%) - 橙色
        plt.axhline(y=th_75, color='#FF8C00', linestyle='--', linewidth=1, alpha=0.8,
                    label=f'75% ({th_75:.2f})')
        # 3. 高风险线 (95%) - 红色
        plt.axhline(y=th_95, color='#FF0000', linestyle='--', linewidth=1.2, alpha=0.9,
                    label=f'95% ({th_95:.2f})')
        # ---------------------------------------
        ...
        # 调整图例位置，避免遮挡
        plt.legend(loc='upper right', fontsize='small', framealpha=0.6)
